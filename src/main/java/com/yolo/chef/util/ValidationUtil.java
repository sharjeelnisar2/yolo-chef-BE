...
public class ValidationUtil {
    ...
    public void validateNewUser(LoginRequest user) {
        ...
        if (doesUsernameExist(user.username())) {
            throw new UserInvalidException(ApiMessages.USERNAME_ALREADY_EXISTS_ERROR.getMessage(),"User with username : " + user.username() + " already exists");
        }
        if (doesEmailExist(user.email())) {
            throw new UserInvalidException(ApiMessages.USER_EMAIL_ALREADY_EXISTS_ERROR.getMessage(),"User with email : " + user.email() + " already exists");
        }
